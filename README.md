...
程序会在后台按节点配置的顺序，逐条探测每个 Emby 节点的上游线路，用于在管理界面显示延迟走势、
上游线路状态和 24 小时可用率。

- 探测周期为 1 分钟，每条上游线路各发起一次 `GET /emby/System/Info/Public`，超时 8 秒。
- 采样只保存在内存中，保留 24 小时，**不写入数据库**，重启后重新累积。
- 在管理界面点「检测」时，除了走一遍完整代理路径，也会立即补一次各条上游线路的探测，
  这样详情页里每条线路的延迟会马上刷新，不用等下一个周期。

如果不希望程序主动访问上游，请注意这部分流量是常驻的：每条上游线路每天约 1440 次请求。

## 控制台日志保留量

控制台日志除了留在内存里，还会以 JSONL 形式落盘到 `data/console-logs.jsonl`，写满一个文件后轮转成
`console-logs.1.jsonl`、`console-logs.2.jsonl`……超出保留数量的最旧文件会被删除。面板里检索日志时会一并
搜索这些落盘文件。

保留量在「配置 → 日志设置 → 日志落盘保留量」里调整：

| 配置项 | 默认 | 范围 | 说明 |
| --- | --- | --- | --- |
| 单个日志文件条数 | 2000 | 200 - 100000 | 单个文件写满多少条后轮转 |
| 保留文件数 | 20 | 1 - 200 | 保留多少个轮转文件 |

可保留的日志总条数约等于两者相乘，默认约 4 万条。保存后立即生效，不需要重启；调小保留量时超出范围的
旧文件会被清理，范围内的历史日志会保留。日志文件按条数而不是按体积轮转，实际占用的磁盘空间取决于日志
行的长度。
...
1. 登录管理界面，打开“配置 → 安全设置 → 管理员双重验证”。
2. 输入 `ADMIN_TOKEN`，使用验证器应用扫描二维码。
3. 输入应用生成的 6 位动态验证码，确认绑定。

开启后，登录管理界面时需要同时输入 `ADMIN_TOKEN` 和动态验证码。也可以在同一位置重新绑定或关闭 2FA。

启用 2FA 后如果更改 `ADMIN_TOKEN`，需要重新设置 2FA。
